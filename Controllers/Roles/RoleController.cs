...
namespace CKM_ManagementSystem.Controllers
{
    public class RoleController : Controller
    {
        ...
        public RoleController(RoleBL roleBL, CurrentUserPermission permission)
        {
            _roleBL = roleBL;
        ...
        public IActionResult RoleEntry(string? id)
        {
            ...
            model.CanWrite = _permission.CanWrite(MenuIDs.Role);
        ...
        [HttpGet]
        [Authorize(Policy = "Permission.Role.Read")]
        public IActionResult RoleList(string searchKeyword, int? status, int pageNumber = 1, int pageSize = 6)
        {
            var model = _roleBL.GetRoleListPaged(pageNumber, pageSize, searchKeyword, status);

            model.CanWrite = _permission.CanWrite(MenuIDs.Role);
            model.CanDelete = _permission.CanDelete(MenuIDs.Role);
        ...
        [HttpPost]
        [Authorize(Policy = "Permission.Role.Write")]
        [ValidateAntiForgeryToken]
        public IActionResult RoleEntry(RoleEntryViewModel model)
        {
            ...
            if (!model.IsEdit && _roleBL.IsRoleCodeDuplicate(model.RoleCode))
            {
                ...
                {
                    success = false,
                    message = $"Role Code '{model.RoleCode}' already exists."
                });
            }
            ...
            if (model.IsEdit)
            {
                ...
                if (existingRole != null &&
                    string.Equals(existingRole.DisplayName, model.DisplayName, StringComparison.OrdinalIgnoreCase))
                {
            ...
            if (checkDuplicateName && _roleBL.IsRoleNameDuplicate(model.DisplayName))
            {
                ...
                {
                    success = false,
                    message = $"Role Name '{model.DisplayName}' already exists."
                });
            }
            ...
            var permissions = model.MenuPermissions?
            ...
            {
                success = false,
                message = !string.IsNullOrEmpty(result)
                    ? result
